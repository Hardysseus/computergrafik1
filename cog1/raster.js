...
function(exports, shader, framebuffer, data) {
	...
	function drawLineBresenham(startX, startY, startZ, endX, endY, endZ, color, storeIntersectionForScanlineFill, edgeStartVertexIndex, edgeEndVertexIndex, edgeStartTextureCoord, edgeEndTextureCoord) {
        ...
        // ESA 2 René Rose, rene.rose@rundwerk.de
        // Programmiert nach Algorithmus-Beschreibung Modul Computergraphik 1, Kap. 3.2.3. Tabelle 3.1.
        ...
        if((startX == endX) && (startY == endY)){
            // Wenn Anfang und Ende von x gleich sind (vertikale Line immer noch möglich) und Anfang y ist gleich Ende y (vertikal nicht mehr möglich)
            // dann ist es ein Punkt
            return;
        }

        // Ermitteln der treibenden Variable
        // Die stärker steigende Achse ist die treibende
        if(dXAbs >= dYAbs){
            // x is driving variable.
            //console.log("x is driving variable");
            // Initialisierung der Entscheidungsvariablen e
            // Vom Betrag der treibenden Variablen x wird 2 mal der Betrag der Variablen y abgezogen
            e = dXAbs - dYAbs2;

            // Beginn des setzens der Punkte
            while(x != endX){ // Endpunkt ist nicht erreicht
                x = x + dXSign; // x wird aktualisiert
                // Prüfung der Entscheidungsvariablen
                if(e>0){
                    // x ist treibend und der Entscheidungspunkt liegt oberhalb der Linie (e > 0)
                    // so bleibt y = y;
                    e = e - dYAbs2;
                }
                else{
                    // e < 0, d.h. die Linie liegt über dem Entscheidungspunkt
                    y = y + dYSign;
                    e = e + dXdYdiff2;
                }
                // Do not add intersections for points on horizontal line
                // and not the end point, which is done in scanline.
                // Prüfung auf horizontale Linie und Startpunkt
                if(startY !== endY || startX !== x){
            ...
            // Initialisierung der Entscheidungsvariablen e
            // Vom Betrag der treibenden Variablen y wird 2 mal der Betrag der Variablen x abgezogen
            e = dYAbs - dXAbs2;

            // Beginn des setzens der Punkte
            while(y != endY){ // Endpunkt ist nicht erreicht
                // festlegen, ob y increment oder dekrement ist
                y = y + dYSign;
                // Prüfung der Entscheidungsvariablen
                if(e>0){
                    // y ist treibend und der Entscheidungspunkt liegt oberhalb der Linie (e > 0)
                    //bleibt x = x;
                    e = e - dXAbs2;
                }
                else{
                    // e < 0, d.h. die Linie liegt über dem Entscheidungspunkt
                    x = x + dXSign;
                    e = e + dYdXdiff2; // e wird aktualisiert
                }
                framebuffer.set(x, y, getZ(x, y), color);
            }
